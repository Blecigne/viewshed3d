...
#' described by Beckers and Beckers (2012). This ensures that every sightline
#' explores a similar portion of the 3d space.
#'
#' @references Beckers, B., and Beckers, P. (2012). A general rule for disk and
#' hemisphere partition into equal-area cells. Computational Geometry, 45(7),
#' 275-283.
#'
...
viewsheds=function(data,positions,angular_res,vox_res,cut_off,pb){
  ...
  if(180%%angular_res==0){ # build the entire theta range
    param <- data.table::data.table(theta=seq(0,180,angular_res),N=NA)
  }else{
    param <- data.table::data.table(theta=seq(0,180+angular_res,angular_res)
                                    ,N=NA)
  }
  #- computes the cumulated number of cells after addition of a new ring
  #- (i.e. theta) based on previous ring. See Beckers and Beckers 2012 for
  #- details
  param[, N:=round(Reduce(function(prev,x){((sqrt(prev)+sqrt(pi))^2)},
                          x =  utils::tail(param$N, -1),
                          init = 1,
                          accumulate = TRUE))]

  # transform cumulated number of cells into the number of cells in one row
  param[2:nrow(param), N:=N-param$N[1:(nrow(param)-1)]]
  param[theta > 90, N := (max(N)-N)+1]
